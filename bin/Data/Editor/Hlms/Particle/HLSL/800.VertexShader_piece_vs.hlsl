...
@property(particleWorldEnabled)
    ...
    uint quadIndex = inVs_vertexId / 4;
    ...
    uint currentBucketCount = entry.lastParticleIndex % @value(BucketSize);
    ...
    float4 spriteTexCoords = emitterCore.spriteCoordRange[particle.spriteNumber];
    ...
    float3 cameraRight = float3(passBuf.particleCameraViewMatrix[0][0], passBuf.particleCameraViewMatrix[1][0], passBuf.particleCameraViewMatrix[2][0]);
    float3 cameraUp = float3(passBuf.particleCameraViewMatrix[0][1], passBuf.particleCameraViewMatrix[1][1], passBuf.particleCameraViewMatrix[2][1]);
    float3 cameraFront = -float3(passBuf.particleCameraViewMatrix[0][2], passBuf.particleCameraViewMatrix[1][2], passBuf.particleCameraViewMatrix[2][2]);
    ...
    // outVs.gl_Position = float4(particle.pos + cameraRight * cornerPos.x + cameraUp * cornerPos.y, 1.0);
    outVs.gl_Position = float4(particle.pos + pX * cornerPos.x + pY * cornerPos.y, 1.0);
	@property( !hlms_identity_world )
        outVs.gl_Position = mul( outVs.gl_Position, worldViewProj );
	@end
